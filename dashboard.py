        ...
        for dev in pending_devices:
            ...
            with col3:
                if st.button("✅ Approve", key=f"approve_{dev.get('mac')}"):
                    dev["approved"] = True
                    save_approved_devices(devices)
                    st.success(f"Device {dev.get('mac')} disetujui ✅")
                    st.experimental_rerun()
                if st.button("❌ Reject", key=f"reject_{dev.get('mac')}"):
                    devices.remove(dev)
                    save_approved_devices(devices)
                    st.warning(f"Device {dev.get('mac')} ditolak ❌")
                    st.experimental_rerun()
    else:
        ...
        for dev in approved_devices:
            ...
            with col2:
                if st.button("❌ Reject", key=f"unapprove_{dev.get('mac')}"):
                    dev["approved"] = False
                    save_approved_devices(devices)
                    st.warning(f"Device {dev.get('mac')} diubah menjadi pending ❌")
                    st.experimental_rerun()
    else:
