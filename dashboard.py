...
# ======================
    ...
    # ======================
    # Topology Script (FULL dari skrip kamu sebelumnya)
    # ======================
    ...
    if not st.session_state.do_search or not search_nodes:
        ...
        with st.spinner("⏳ Sedang memuat data dan membangun topology..."):
            ...
            # === kode lengkap topology dari skrip kamu ===
            # --- mulai ---
            ...
            if df_filtered.empty:
                ...
                for ring in ring_ids:
                    ...
                    for nid in nodes_order:
                        info = {"Fiber Type": "", "Site Name": "", "Host Name": "", "FLP Vendor": ""}
                        df_match = ring_df[(ring_df[col_site].astype(str).str.strip() == nid) | (ring_df[col_dest].astype(str).str.strip() == nid)]
                        if not df_match.empty:
                            row0 = df_match.iloc[0]
                            info = {
                                "Fiber Type": str(row0.get(col_fiber, "")),
                                "Site Name": str(row0.get(col_site_name, "")),
                                "Host Name": str(row0.get(col_host, "")),
                                "FLP Vendor": str(row0.get(col_flp, ""))
                            }
                        ...
                        label_parts = [fiber, nid, info["Site Name"], info["Host Name"], info["FLP Vendor"]]
                        label_parts = [p for p in label_parts if p]
                        title = "<br>".join(label_parts)
                        x, y = positions.get(nid, (0,0))
                        font_color = "red" if any(re.search(re.escape(kw), nid, re.IGNORECASE) for kw in search_nodes) else "black"

                        net.add_node(
                            nid, label="\n".join(label_parts),
                            x=x, y=y, physics=False, size=50, shape="image",
                            image=node_image,
                    ...
                    html_str = html_str.replace('<body>', '<body><div class="canvas-border"><style>.vis-network{background-image: linear-gradient(to right, #d0d0d0 1px, transparent 1px), linear-gradient(to bottom, #d0d0d0 1px, transparent 1px); background-size: 50px 50px;}</style>')
